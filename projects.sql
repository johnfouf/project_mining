...
create temp table matched_undefined_wt_only as select distinct docid, "40:wt__________::unidentified" as id, prev||' '||middle||' '||next as context from (setschema 'docid,prev,middle,next'
...
select jdict('documentId', docid, 'projectId', id, 'confidenceLevel', 0.8) as C1, docid, id from (
...
select jdict('documentId', docid, 'projectId', id, 'confidenceLevel', sqroot(min(1.49,confidence)/1.5)) as C1, docid, id from ( select docid,id,max(confidence) as confidence, small_string, string from ( select docid, id,
    (fullprojectmatch*10
    +coreprojectmatch*10
...
select jdict('documentId', docid, 'projectId', id, 'confidenceLevel', sqroot(min(1.49,confidence)/1.5)) as C1, docid, id from ( select docid,id,max(confidence) as confidence from ( select docid, id,
...
delete from matched_undefined_wt_only where docid in (select docid from output_table where regexprmatches('wt__________::', id));
